...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents lblGCash As System.Web.UI.WebControls.Label
    Protected WithEvents lblChange As System.Web.UI.WebControls.Label
    Protected WithEvents lblCashier As System.Web.UI.WebControls.Label
    ...
    Protected WithEvents litCompanyName As New Literal
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            ...
            Dim companyCode As String = Trim(objReader("company"))
            LoadCompanyName(companyCode)

            lblPrintedOn.Text = Date.UtcNow.AddHours(8)
            lblCustomerName.Text = Trim(objReader("client"))
            lblContactNo.Text = Trim(objReader("contact_no"))
            lblSINo.Text = Trim(objReader("so_no"))

            lblDiscountedAmount.Text = Trim(objReader("less"))
            lblNetAmount.Text = Trim(objReader("net_sales"))

            lblCash.Text = Trim(objReader("paid_amount"))
            lblChange.Text = Trim(objReader("change_amount"))
            lblCashier.Text = Trim(objReader("su_firstname"))
            lblPayType.Text = Trim(objReader("pos_type"))
            lblRefNo.Text = Trim(objReader("pos_refno"))
            lblNotes.Text = Trim(objReader("pos_notes"))

            objReader.Close()

            objCommand = New SqlCommand("Inv_PrintPreview_POSCompanyLogo", objConnection)
            objCommand.CommandType = CommandType.StoredProcedure
            objCommand.Parameters.Add("@id", Request("id"))
            ...
            htmlImgCompanyLogo = "<img src=" & Trim(objReader("logo_path")) & ">"
...
Handles dgList.ItemCreated
    ...
    Private Sub LoadCompanyName(ByVal companyCode As String)

        Dim myItems As String = String.Empty

        If companyCode = "MIMI" Then
            myItems &= ("<span>MIMI YUAN SU</span>" & _
                                    "<br>" & _
                                        "<span>SHOPPING CENTER</span>" & _
                                    "<br>")
        Else
            myItems &= ("<span>BETTERBUY HOUSEHOLD</span>" & _
                                                "<br>" & _
                                                    "<span>APPLICANCES TRADING</span>" & _
                                                "<br>")
        End If


        litCompanyName.Text = myItems
