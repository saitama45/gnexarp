...
Public Class inv_issuance_list
...
#End Region
    ...
    Private Sub DataGrid_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid.ItemCommand
        ...
        If e.CommandName = "Open" Then
            ...
            Response.Redirect("inv_dritems_list?&id=" & strKeyFieldValue & "&sys_table=" & strStoredProc)
    ...
    Private Sub Refresh_CurrentTerms()
        ...
        objConnection.Close()
    End Sub
    Private Sub Refresh_Company()
        Dim objConnection As New SqlConnection(ConfigurationSettings.AppSettings("appConn"))
        objConnection.Open()

        Dim objCommand As New SqlCommand("Inv_SONum_Get", objConnection)
        objCommand.CommandType = CommandType.StoredProcedure
        ...
        lblSONo.Text = Trim(objReader("so_no_show"))
        lblSODate.Text = Date.UtcNow.AddHours(8).ToShortDateString()
