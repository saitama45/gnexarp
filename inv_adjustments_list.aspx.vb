...
Public Class inv_adjustments_list
...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents txtSOH As System.Web.UI.WebControls.TextBox
    ...
    Protected WithEvents lblVariance As System.Web.UI.WebControls.Label
    ...
    Protected WithEvents ddlSKUCode As System.Web.UI.WebControls.DropDownList
...
#End Region
    ...
    Public strSubMenuName As String = "inv_adjustments_list"
    Public strStoredProc As String = "inv_adjustments_list"
    Public strStoredProcPageCount As String = "Inv_AdjustmentPageCount_Get"
    ...
    Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            ...
            'btnAddRecord.Text = "Add Adjustment"
    ...
    Private Sub btnAddRecord_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        ...
        Dim objCommand As New SqlCommand("Ref_Warehouse_Get_droplist", objConnection)
        objCommand.CommandType = CommandType.StoredProcedure
        objCommand.Parameters.Add("@Session_username", Session("username"))
        Dim objReader As SqlDataReader = objCommand.ExecuteReader
    ...
    Private Sub txtSOH_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSOH.TextChanged

        If chbItemNotFound.Checked = True Then
            lblVariance.Text = txtSOH.Text
            lblCurrentSOH.Text = 0
        Else
            Refresh_Variance()
            Refresh_CurrentSOH()
        End If
    ...
    End Sub
    Private Sub Refresh_Variance()
        lblVariance.Text = txtSOH.Text - lblCurrentSOH.Text
    End Sub
    Private Sub Refresh_CurrentSOH()
        ...
        objCommand.Parameters.Add("@id", ddlItem.SelectedValue)

        Dim objReader As SqlDataReader = objCommand.ExecuteReader
        objReader.Read()

        lblCurrentSOH.Text = Trim(objReader("balance"))
    ...
    Private Sub ClearInputs()
        ...
        calDate.SelectedDate = Date.UtcNow.AddHours(8).ToShortDateString
        ...
        ddlSKUCode.Visible = False

        lblVariance.Text = 0
        lblCurrentSOH.Text = 0

        chbItemNotFound.Visible = False
    ...
    Private Function SaveInput()
        ...
        'the stored procedure and parameters
        Dim objCommand As New SqlCommand("Inv_Adjustments_Save", objConnection)
        objCommand.CommandType = CommandType.StoredProcedure
        ...
        objCommand.Parameters.Add("@adj_date", calDate.SelectedDate)

        If chbItemNotFound.Checked = True Then
            objCommand.Parameters.Add("@rm_code", ddlSKUCode.SelectedValue)

        Else
            objCommand.Parameters.Add("@ud_id", ddlItem.SelectedValue)
        ...
        If chbNoExpiryDate.Checked = True Then
            objCommand.Parameters.Add("@new_expiry_date", "1/1/1900")
        Else
            objCommand.Parameters.Add("@new_expiry_date", calExpiryDate.SelectedDate)
        End If

        objCommand.Parameters.Add("@soh", txtSOH.Text)
        objCommand.Parameters.Add("@current_soh", lblCurrentSOH.Text)
        objCommand.Parameters.Add("@variance", lblVariance.Text)
        objCommand.Parameters.Add("@entry_type", ddlEntryType.SelectedValue)
        ...
        Try
            ...
            Sys_Class.SaveToAuditTrail(Session("username"), _
                                        ...
                                        strSubMenuName, _
                                        "Date: " + calDate.SelectedDate + " > ItemCode: " + ddlItem.SelectedValue + " > SOH: " + txtSOH.Text + " > CurrentSOH: " + lblCurrentSOH.Text + " > Variance: " + lblVariance.Text + " > Note: " + txtNote.Text, _
                                        objNewID.Value, _
                                        0)
    ...
    Private Function ValidateInput()
        ...
        'the stored procedure and parameters
        Dim objCommand As New SqlCommand("Inv_Adjustments_Validate", objConnection)
        objCommand.CommandType = CommandType.StoredProcedure
        objCommand.Parameters.Add("@FormAction", txtFormAction.Text)
        objCommand.Parameters.Add("@id", lblID.Text)
        objCommand.Parameters.Add("@adj_date", calDate.SelectedDate)
        ...
        If chbItemNotFound.Checked = True Then
            objCommand.Parameters.Add("@isNoItemFound", 1)
            objCommand.Parameters.Add("@item_code", ddlSKUCode.SelectedValue)
        Else
            objCommand.Parameters.Add("@isNoItemFound", 0)
            objCommand.Parameters.Add("@item_code", "")
        End If
    ...
    Private Sub Refresh_Items()
        ...
        Dim objCommand As New SqlCommand("Ref_WarehouseItems_Get_droplist", objConnection)
        objCommand.CommandType = CommandType.StoredProcedure
    ...
    Private Sub chbItemNotFound_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chbItemNotFound.CheckedChanged
        ...
        If chbItemNotFound.Checked = True Then
            ...
            Dim objReader As SqlDataReader = objCommand.ExecuteReader
            ddlSKUCode.DataSource = objReader
            ddlSKUCode.DataValueField = "code"
            ddlSKUCode.DataTextField = "name"
            ddlSKUCode.DataBind()
            ddlSKUCode.Items.Insert(0, "")
            ddlSKUCode.SelectedIndex = 0
            objReader.Close()
            objConnection.Close()
            ddlSKUCode.Visible = True

            txtSerial.Text = ""
            txtSerial.Visible = True

            chbNoExpiryDate.Visible = True
